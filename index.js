...
function processSum(numberList, callback) {
  const listSum = numberList.reduce((accumulator, currentValue) => {
...
function getFullNames(runners) {
  let names = [];  
  runners.forEach((items) => {names.push(`${items.last_name}, ${items.first_name}`)})
  return names;
}
...
function firstNamesAllCaps(runners) {
  let names = [];
  runners.forEach((items) => {names.push(`${items.first_name.toUpperCase()}`)})
  return names;
}
...
function tallyUpDonations(runners) {
  let total = runners.reduce((accumulator, currentValue) => {
